...
echo "──────────────────────────────────────"
echo "  Mongo Express Vault Secret Loader      "
echo "──────────────────────────────────────"
...
export ME_CONFIG_MONGODB_ADMINPASSWORD="$MONGO_PASSWORD"
export ME_CONFIG_MONGODB_URL="mongodb://${MONGO_USER}:${MONGO_PASSWORD}@mongodb:27017/"
export ME_CONFIG_BASICAUTH=false

[ -z "$MONGO_USER" ]     && echo "❌ MONGO_USER is empty"     && exit 1
[ -z "$MONGO_PASSWORD" ] && echo "❌ MONGO_PASSWORD is empty" && exit 1

echo "✅ Secrets loaded for mongo-express successfully"

# ── Start Mongo express ───────────────────────────────────
echo "Starting Mongo express express..."
